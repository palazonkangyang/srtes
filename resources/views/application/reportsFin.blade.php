...
<div class="row pos-rel">
    <div class="col-md-12"><h4 class="page-head-line">Finance Reports</h4></div>
...
<div id="no-more-tables">
 <table class="col-md-12 table-bordered table-striped table-condensed cf" id="xls-table">
    <thead class="cf">
        ...
        <th>{!!$reports->sortColumn('pp_status','PP Status')!!}</th>
              ...
              @if($report->status == 1)
               @if($report->pp_status == 0) 
                ...
                @elseif($report->pp_status == 2)
                    <span class="alert-black">Export</span>
    ...
    <div class='hide'>{{$grant_total =$grant_total + $report->total }}</div>
        ...
        @endforeach
    <td></td><td></td> <td></td><td></td><td></td>  <td></td> <td>Grant Total:</td><td>{{$grant_total}}</td>
    </tbody>
</table>
</div>
</div>
