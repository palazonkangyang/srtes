<div style="width:500px; margin:0 auto; border:1px solid black; padding:20px;">
	<p style="font-family: Arial, sans-serif; font-size:13px; line-height:22px;">
		Dear {{$receiver_name}},
	</p>
	<br />
	<p style="font-family: Arial, sans-serif; font-size:13px; line-height:22px;">
		{{ $sender_name }} has forwarded the case #{{$case_number}} to you. You can now have the options to approve or reject the case. For more details, please click <a href="{{ $url }}" target="_blank">here</a>.
	</p>
	<br />
	<p style="font-family: Arial, sans-serif; font-size:13px; line-height:22px;">
		Approval Management System<br />Singapore Red Cross
	</p>
</div>