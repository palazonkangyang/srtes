...
class ApplicationPresenter extends PresenterCore
{
	...
	public function ApplicationProcess(Request $request){
		...
		if($request->get('type_of_request') && $request->get('forms')){
			 ...
			 */
			if($form['0']->id == 1 || $form['0']->id == 15 || $form['0']->id == 16) {
			...
			foreach($set as $appr){
				foreach($appr['approvers'] as $key => $app ){
                                 ...
                                 $list[0] = $getRO;  
                                  ...
                                  /* }else if($form['0']->id == 15){
                                       
                                       
                                            if($department['0']->idsrc_departments ==5)
                                     {
                                         ...
                                         {
                                            return redirect('/application/new/process')->with('error_message', 'Requester should be HOD and above');
                
                                         }
                                     }else{
                                   if($department['0']->dept_head == \Auth::user()->idsrc_login){
                                                 if($getRO !=$getSecGen || $getRO!=$getDSG)
                                                {
                                                    $list[0] = $getRO;
                                             $list[1] = $getHrHead;
                                                }else{
                                 ...
                                 /*  else if($form['0']->id == 17 || $form['0']->id == 18){
                                      	if(isset($getHod)){
							$list[0] = $getHod;
            ...
            if($list){
                    ...
                    if($form['0']->id == 12 || $form['0']->id ==13|| $form['0']->id ==14 || $form['0']->id ==15 || $form['0']->id ==16|| $form['0']->id ==19 || $form['0']->id ==20)
                    {
    ...
    public function pending()
    {
        ...
        $firstprepare = ModelFactory::getInstance('Approver')
         ->leftjoin('ams_applications', 'ams_applications.id', '=', 'ams_approver_person.app_id')
            ...
            ->whereNotIn('ams_applications.status', [ 1, 2, 3 ])    
