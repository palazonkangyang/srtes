...
class AccountController extends ControllerCore
{
   ...
   public function logout(Request $request){
    ...
    {
      $temp_approver_id = $_POST['temp_approver_id'];
      $id = Auth::User()->idsrc_login;

      $user = ModelFactory::getInstance('User')->find($id);
      $user->temp_approver_id = $temp_approver_id;

      if($user->save())
      {
        $result = ModelFactory::getInstance('TempApproverUser');

        $result->start_datetime = Carbon::now();
        $result->user_id = $id;
        $result->temp_approver_id = $temp_approver_id;
        $result->save();
